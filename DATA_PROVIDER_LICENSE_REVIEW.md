...
| [Pappalardo et al., Soccer match event dataset](https://figshare.com/articles/dataset/Events/7770599), CC BY 4.0 | Incluye eventos de tarjetas con identificadores de jugador y distingue amarilla, roja y segunda amarilla. La licencia permite reutilización con atribución. | No seleccionado como fuente de carrera: cubre una temporada de cinco grandes ligas más torneos concretos, por lo que no demuestra cobertura histórica mundial ni permite por sí solo construir las dos clasificaciones de carrera. |
...
Como excepción revisada, cinco etiquetas con contexto histórico suficientemente distintivo quedaron documentadas en el código de consolidación: `Ramos S.` → Sergio Ramos, `Ronaldo C.` → Cristiano Ronaldo, `Villa D.` → David Villa, `Motta T.` → Thiago Motta y `Suarez L.` → Luis Suárez. La revisión se apoyó en las temporadas y competiciones conservadas en la evidencia del snapshot, no en el nombre abreviado por sí solo. Las demás etiquetas siguen separadas hasta disponer de un identificador estable o de una segunda señal independiente; estos enlaces no corrigen la cobertura incompleta ni cambian el estado `draft`.

También se revisó el conjunto de eventos de Pappalardo et al. Está publicado bajo CC BY 4.0 y ofrece mejores identificadores de jugador, pero su alcance es de una temporada y un conjunto limitado de competiciones; sirve como contraste o posible enriquecimiento acotado, no como sustituto homogéneo de las tarjetas globales de carrera.

RSSSF mantiene una referencia muy útil para contrastar goles históricos: su tabla distingue “Official Matches” y “Top level”, pero también advierte de límites inferiores, datos preliminares y criterios que pueden incluir selecciones no absolutas. Además, la propia página conserva la indicación de copyright y “All rights reserved”, por lo que se mantiene como referencia de auditoría y no como dataset redistribuible para `player-career-goals` sin permiso adicional. La definición de Rango90 seguirá siendo únicamente clubes + selección absoluta.
