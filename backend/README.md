...
La decisión de fuente histórica de Champions está en [BLOQUE 8](audits/block8/champions-historical-source-policy.md). Transfermarkt y API-Football/API-Sports quedan congeladas como fuentes no publicables para `uefa-champions-league-goals`; sus snapshots existentes permanecen en `draft`. La prueba 7E observó API-Football únicamente entre 2011 y 2026, por lo que ese rango no puede presentarse como histórico desde 1955/56. Una eventual categoría 2011–2026 tendría que usar otro slug, definición, snapshot y derechos. No ejecutar el importador de Champions para esta categoría hasta seleccionar una fuente que cumpla la matriz de requisitos y exista autorización escrita para rankings derivados.
...
La sincronización de Champions queda suspendida para la categoría histórica: no se deben ejecutar nuevas peticiones masivas, seeds, imports ni reconstrucciones con API-Football hasta resolver la fuente. Los snapshots anteriores se conservan y el laboratorio puede seguir mostrándolos con advertencia de provisionalidad; `official` no puede servirlos.
