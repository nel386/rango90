...
if (!isolatedUrl) {
  console.log(JSON.stringify({ status: 'not_run', integration: 'block21-world-cup-missing-snapshot', reason: 'RANGO90_ISOLATED_DATABASE_URL_missing' }));
  process.exit(0);
}
...
try {
  ...
  console.log(JSON.stringify({ status: 'passed', integration: 'block21-world-cup-missing-snapshot', fallback: 'none' }, null, 2));
} finally {
  await app.close();
  await db.end();
}
