...
try {
  ...
  const historical = await lab.inject({ method: 'GET', url: '/v1/rankings/uefa-champions-league-assists?dataset=historical_base' }); assert.equal(historical.statusCode, 404);
