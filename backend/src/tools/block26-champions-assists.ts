...
const apiReportFile = process.env.BLOCK27_API_REPORT_FILE?.trim() || process.env.BLOCK26_API_REPORT_FILE?.trim() || '';
...
function rowFact(row: Record<string, unknown>): ChampionsAssistFact {
  ...
  const player: PlayerIdentity = {
    ...
    sourceKey: String(row.source_key), sourcePlayerId: String(row.source_player_id), resolution: 'explicit'
  };
  return {
...
async function persistSnapshot(pool: pg.Pool, snapshot: ChampionsAssistSnapshot, action: 'created' | 'rollback_requested' = 'created'): Promise<{ inserted: boolean; entries: number }> {
...
async function main(): Promise<void> {
  ...
  const historical = await readInput(historicalFactsFile); const active = await readInput(activeFactsFile); const apiReport = await readOptionalJson(apiReportFile); const previousReport = await readOptionalJson(resolve(outputRoot, artifact('REPORT.json')));
  const pool = new pg.Pool({ connectionString: databaseUrl, max: 2, connectionTimeoutMillis: 5_000 });
  try {
    await pool.query('BEGIN');
    const incoming = [...historical.facts, ...active.facts];
    ...
    const activeCoverageWasConfirmed = active.activeCoverageComplete ?? activeCoverageComplete;
    ...
    const coverageMatrix = [...Array.from({ length: CHAMPIONS_ASSISTS_HISTORICAL_END - CHAMPIONS_ASSISTS_HISTORICAL_START + 1 }, (_, index) => CHAMPIONS_ASSISTS_HISTORICAL_START + index), activeSeasonStart].map((season) => ({ season, dataset: season === activeSeasonStart ? 'active_season_weekly' : 'historical_base', status: season === activeSeasonStart ? (activeSnapshot.coverageComplete ? 'covered' : 'pending') : (historicalSeasons.has(season) ? 'covered' : 'pending'), facts: facts.filter((fact) => fact.edition.seasonStart === season).length }));
    ...
    const report: JsonRecord = { artifactKind: `${artifactPrefix.toLocaleLowerCase()}_champions_assists`, reportVersion: '1', generatedAt: now, commit: process.env.GITHUB_SHA ?? 'local', workflowRun: process.env.GITHUB_RUN_ID ?? null, status: activeSnapshot.coverageComplete || historicalSnapshot.coverageComplete ? 'passed_with_gates' : 'not_sufficient', readyForApproval: false, category: CHAMPIONS_ASSISTS_CATEGORY_SLUG, metric: 'assists', facts: { before: before.length, incoming: incoming.length, added: persistence.added, skipped: persistence.skipped, after: facts.length, totalAssists: facts.reduce((sum, fact) => sum + fact.assists, 0), primary: primaryFacts, contrast: contrastFacts, conflicts: imported.conflicts.length, unresolvedIdentities: activeSnapshot.unresolvedIdentityFacts.length + historicalSnapshot.unresolvedIdentityFacts.length, unverified: activeSnapshot.excludedUnverifiedFacts.length + historicalSnapshot.excludedUnverifiedFacts.length }, sources: [...new Set(facts.map((fact) => fact.sourceKey))].sort(), conflicts: imported.conflicts, apiFootball: apiReport, eventsWithoutPlayer: apiReport.uncreditedGoalEvents ?? 0, coverage: { historical: historicalSnapshot.coverage, historicalCandidateStatus: historicalSnapshot.metadata.candidateStatus, active: activeSnapshot.coverage, activeCandidateStatus: activeSnapshot.metadata.candidateStatus }, coverageMatrix, snapshots: { historical: { id: historicalSnapshot.id, contentSha256: historicalSnapshot.contentSha256, persisted: historicalPersisted }, active: { id: activeSnapshot.id, contentSha256: activeSnapshot.contentSha256, parentSnapshotId: activeSnapshot.parentSnapshotId, persisted: activePersisted } }, weeklyChanges: changes, idempotency: { addedOnRerun: rerun.added.length, skippedOnRerun: rerun.skipped.length, conflictsOnRerun: rerun.conflicts.length, sameSnapshotHash: Boolean(priorActive && priorActive.id === activeSnapshot.id), duplicateNewFacts: rerun.added.length }, rollback: { id: rollback.snapshot.id, rollbackOf: rollback.snapshot.rollbackOf, persisted: rollbackPersisted, status: 'passed_fixture', published: false }, official: { status: 'blocked', snapshotCreated: false }, production: { databaseAccess: 'none', renderTouched: false, imagesTouched: false, rightsChanged: false }, rawPayloadsStored: false };
