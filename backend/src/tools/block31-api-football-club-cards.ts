...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (plans.length === 0) for (const competition of COMPETITIONS) { const result = await request(`/leagues?id=${competition.id}`); requests.push(result.evidence); plans.push(coverageFromLeague(competition, result.body)); }
    ...
    if (planOnly) { await writeJson(resolve(outputRoot, artifact('REPORT.json')), { artifactKind: 'block31_api_football_club_cards', status: 'plan_ready', activeSeason, requestsPerformed: requests.length, quotaRemaining: requests.at(-1)?.quotaRemaining ?? null, competitions: COMPETITIONS, coverageMatrix, factsImported: 0, rawPayloadsStored: false, secretPrinted: false }); return; }
    ...
    const report = { artifactKind: 'block31_api_football_club_cards', status: stoppedForQuota ? 'partial' : facts.length > 0 ? 'ready_for_lab_load' : 'not_sufficient', activeSeason, requestsPerformed: requests.length, quotaRemaining: requests.at(-1)?.quotaRemaining ?? null, quotaReserve, competitions: COMPETITIONS, coverageMatrix, factsImported: facts.length, playersWithFacts: new Set(facts.map((fact) => fact.canonicalPlayerId)).size, unresolvedFacts: facts.filter((fact) => !fact.canonicalPlayerId).length, sourceType: 'primary', rawPayloadsStored: false, secretPrinted: false, officialSnapshotCreated: false, redTypesDifferentiated: facts.some((fact) => fact.redSecondYellow !== null || fact.redDirect !== null) };
