...
async function main(): Promise<void> {
  ...
  try {
    ...
    let stoppedForBudget = false; let providerError = false;
    if (plans.length === 0) for (const competition of scopedCompetitions) {
      if (maxRequests === 0 || requests.length >= maxRequests) { stoppedForBudget = true; break; }
      const result = await request(`/leagues?id=${competition.id}`); requests.push(result.evidence); if (!result.evidence.ok) providerError = true; plans.push(coverageFromLeague(competition, result.body));
    }
    ...
    for (const plan of plans) {
      ...
      for (let page = 1; ; page += 1) {
        ...
        const result = await request(`/players?league=${plan.competition.id}&season=${activeSeason}&page=${page}`); requests.push(result.evidence); if (!result.evidence.ok) providerError = true;
      ...
      if (matrix) { matrix.yellowCards = facts.filter((fact) => fact.competition.providerId === plan.competition.id).reduce((sum, fact) => sum + (fact.yellowCards ?? 0), 0); matrix.redCards = facts.filter((fact) => fact.competition.providerId === plan.competition.id).reduce((sum, fact) => sum + (fact.redCards ?? 0), 0); matrix.status = matrix.playerPages > 0 && !stoppedForQuota && !stoppedForBudget ? 'complete' : 'partial'; matrix.reason = matrix.status === 'complete' ? 'Todas las páginas de estadísticas de jugadores de la temporada activa fueron consultadas.' : stoppedForBudget ? 'La temporada activa sigue en curso; se conserva el tramo válido observado dentro del presupuesto.' : stoppedForQuota ? 'La cuota impidió completar las páginas.' : 'La respuesta del proveedor fue parcial.'; }
      if (stoppedForQuota) break;
    }
    const unresolvedFacts = facts.filter((fact) => !fact.canonicalPlayerId || fact.verificationStatus !== 'confirmed').length;
    const activeSeasonStatus = stoppedForQuota ? 'quota_insufficient' : providerError ? 'provider_unavailable' : facts.length > 0 && unresolvedFacts === 0 ? 'provisional_active_season' : 'partial_missing_provider_data';
    const report = { artifactKind: 'block31_api_football_club_cards', status: activeSeasonStatus, activeSeasonStatus, seasonInProgress: true, snapshotEligibility: activeSeasonStatus === 'provisional_active_season', stoppedForBudget, stoppedForQuota, providerError, activeSeason, requestsPerformed: requests.length, maxRequests, quotaRemaining: requests.at(-1)?.quotaRemaining ?? null, quotaReserve, competitions: scopedCompetitions, coverageMatrix, observedPages: coverageMatrix.reduce((sum, row) => sum + row.playerPages, 0), factsImported: facts.length, playersWithFacts: new Set(facts.map((fact) => fact.canonicalPlayerId)).size, unresolvedFacts, conflicts: facts.filter((fact) => fact.verificationStatus === 'conflict').length, sourceType: 'primary', rawPayloadsStored: false, secretPrinted: false, officialSnapshotCreated: false, redTypesDifferentiated: facts.some((fact) => fact.redSecondYellow !== null || fact.redDirect !== null) };
