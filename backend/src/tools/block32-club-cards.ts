...
const knownQuota = Math.max(0, Number(process.env.BLOCK32_KNOWN_QUOTA_REMAINING ?? 0) || 0);
...
async function main(): Promise<void> {
  ...
  const report = {
    ...
    status: knownQuota === 0 ? 'quota_insufficient' : plan.batches.some((batch) => batch.status === 'quota_insufficient') ? 'partial_scope' : 'planned',
    ...
    note: knownQuota === 0 ? 'No se hicieron peticiones: la cuota conocida es 0. Se conserva el último snapshot candidato válido.' : 'Plan preparado; la tanda real requiere una ejecución explícita con cuota preautorizada.'
  };
