...
type BaseManifest = { artifactKind: 'block45_base_manifest'; version: '1'; selectedSeasons: number[]; activeSeason: number; discoveredSeasonsByLeague: Record<string, number[]>; coverage: CoverageRow[]; basePlayerSeasonRecords: BasePlayerSeasonRecord[]; basePlayerNames: Array<[string, string]>; facts: YellowCardFact[]; requests: RequestEvidence[]; competitionDecisions: Array<Record<string, unknown>>; generatedAt: string; baseCoverageComplete: boolean };
...
async function main(): Promise<void> {
  ...
  if (mode === 'load' && baseManifestInput) {
    ...
    if (!manifest.baseCoverageComplete) { await writeJson(resolve(outputRoot, 'BLOCK45_REPORT.json'), { ...baseReport, mode, status: 'load_blocked_base_manifest_incomplete', noSnapshotsCreated: true, noDatabaseTouched: true, reason: 'El manifiesto base no contiene las 85 combinaciones completas; no se realizaron peticiones de expansión.' }); return; }
    ...
    for (const league of leagues) for (const season of querySeasons) {
      ...
      for (;;) {
        ...
        try {
          const result = await request(endpoint, 'league_page'); baseRequests.push(result.evidence); if (totalPages === null) totalPages = result.evidence.pagingTotal; rowCoverage.pagesExpected = totalPages; rowCoverage.pagesRead = page;
  ...
  const baseManifest: BaseManifest = { artifactKind: 'block45_base_manifest', version: '1', selectedSeasons, activeSeason, discoveredSeasonsByLeague: Object.fromEntries([...discoveredByLeague.entries()].map(([id, years]) => [id, years])), coverage, basePlayerSeasonRecords, basePlayerNames: [...basePlayerNames.entries()], facts, requests: baseRequests, competitionDecisions: [...competitionDecisions.values()], generatedAt: new Date().toISOString(), baseCoverageComplete: completeBaseCoverage };
  await writeJson(resolve(outputRoot, 'BLOCK45_BASE_MANIFEST.json'), baseManifest);
  ...
  const requestAttempts = baseRequestAttempts + requestAttemptsUsed;
  await writeJson(resolve(outputRoot, 'BLOCK45_REQUESTS.json'), { requests: reportRequests, summary: { recordedFinalResponses: reportRequests.length, actualAttempts: requestAttempts, baseAttempts: baseRequestAttempts, expansionAttempts: requestAttemptsUsed, retryAttempts: reportRequests.reduce((sum, request) => sum + request.retryCount, 0), statusCounts: Object.fromEntries([...new Set(reportRequests.map((request) => request.status))].map((status) => [status, reportRequests.filter((request) => request.status === status).length])) } });
  ...
  const quotaInitial = reportRequests[0]?.quotaRemaining ?? null; const quotaFinal = reportRequests.at(-1)?.quotaRemaining ?? null; const quotaHeaderDelta = quotaInitial !== null && quotaFinal !== null ? quotaInitial - quotaFinal : null; const successfulResponses = reportRequests.filter((request) => request.status >= 200 && request.status < 300).length; const rateLimitedResponses = reportRequests.filter((request) => request.status === 429).length; const retryAttempts = reportRequests.reduce((sum, request) => sum + request.retryCount, 0);
