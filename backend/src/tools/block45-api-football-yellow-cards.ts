...
async function main(): Promise<void> {
  ...
  if (mode !== 'plan') {
    ...
    for (const league of leagues) {
      ...
      try {
        ...
        if (result.evidence.status !== 200 || hasProviderErrors(result.body.errors) || !Array.isArray(result.body.response)) { providerErrors += 1; discoveredByLeague.set(league.id, []); continue; }
  ...
  if (mode === 'plan' || mode === 'continue') {
    ...
    queryCoverage: for (const rowCoverage of queryRows) {
      ...
      for (;;) {
        ...
        if (result.evidence.status !== 200 || hasProviderErrors(result.body.errors) || !Array.isArray(result.body.response)) { providerErrors += 1; rowCoverage.reason = result.evidence.status === 429 ? 'minute_rate_limit_stopped_queue' : hasProviderErrors(result.body.errors) ? 'provider_errors_field_nonempty' : 'provider_response_incomplete'; break; }
  ...
  if (mode === 'expand' && completeBaseCoverage) {
    expansionLoop: for (const { playerId, season, key: coverageKey } of pendingExpansionPairs) {
      ...
      if (result.evidence.status !== 200 || hasProviderErrors(result.body.errors) || !Array.isArray(result.body.response)) { providerErrors += 1; playerSeasonCoverage[coverageKey] = { playerId, season, status: 'provider_error', competitions: [] }; if (stoppedForRateLimit) break expansionLoop; continue; }
      if (result.body.response.length === 0) { playerSeasonCoverage[coverageKey] = { playerId, season, status: 'player_did_not_participate', competitions: [] }; continue; }
      if (rows.some((row) => !Number.isInteger(Number(object(row.player).id)) || Number(object(row.player).id) < 1 || !Array.isArray(row.statistics))) { providerErrors += 1; playerSeasonCoverage[coverageKey] = { playerId, season, status: 'provider_error', competitions: [] }; continue; }
      const returnedCompetitions = new Set<string>(); let expansionFactCount = 0;
      const unclassifiedLegacyCompetitions = new Set<string>();
      for (const row of rows) for (const statistic of array(row.statistics).map(object)) {
        recordCompetitionDecision(statistic);
        const league = object(statistic.league); const leagueId = Number(league.id); const leagueName = String(league.name ?? `Competition ${leagueId}`); const team = object(statistic.team);
      ...
      playerSeasonCoverage[coverageKey] = { playerId, season, status: unclassifiedLegacyCompetitions.size > 0 ? 'pending_legacy_competition' : expansionFactCount > 0 ? 'data_available' : 'provider_returned_no_eligible_club_stats', competitions: [...returnedCompetitions].sort(), ...(unclassifiedLegacyCompetitions.size > 0 ? { unclassifiedLegacyCompetitions: [...unclassifiedLegacyCompetitions].sort() } : {}) };
    }
  }
  ...
  const finalLegacyCompetitionPendingPairs = expansionPairs.filter(({ key }) => playerSeasonCoverage[key]?.status === 'pending_legacy_competition');
  const expansionComplete = completeBaseCoverage && finalLegacyCompetitionPendingPairs.length === 0 && pendingExpansionPairs.every(({ key }) => terminalExpansionStatuses.has(playerSeasonCoverage[key]?.status ?? ''));
