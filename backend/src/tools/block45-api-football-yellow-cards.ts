...
type RequestEvidence = { endpoint: string; status: number; responseSha256: string; quotaRemaining: number | null; pagingTotal: number | null; kind: 'coverage' | 'league_page' | 'player_season' };
...
async function request(endpoint: string, kind: RequestEvidence['kind']): Promise<{ body: Envelope; evidence: RequestEvidence }> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      ...
      const evidence = { endpoint, status: response.status, responseSha256: sha256(raw), quotaRemaining: quota(response.headers), pagingTotal: Number.isInteger(pagingTotal) && pagingTotal > 0 ? pagingTotal : null, kind };
      if ((response.status === 429 || response.status >= 500) && attempt < maxRetries) { await new Promise((resolveDelay) => setTimeout(resolveDelay, 750 * (attempt + 1))); continue; }
...
function isOfficialClubCompetition(statistic: JsonRecord): boolean {
  ...
  if (type === 'international' || /world cup|euro|copa america|nations league|afcon|asian cup|gold cup/u.test(name) || ['world', 'europe'].includes(country)) return false;
  return true;
}
...
async function main(): Promise<void> {
  ...
  const requests: RequestEvidence[] = []; const facts: YellowCardFact[] = []; const discoveredByLeague = new Map<number, number[]>(); const playerIds = new Set<string>(); let providerErrors = 0; let stoppedForBudget = false;
  ...
  const expansionEstimate = playerIds.size * selectedSeasons.length;
  ...
  await writeJson(resolve(outputRoot, 'BLOCK45_REQUESTS.json'), { requests });
  ...
  const totalEstimated = discoveredByLeague.size + pagesEstimated + expansionEstimate;
  const notQueried = leagues.flatMap((league) => discoveredByLeague.get(league.id) ?? []).filter((season) => !requestedSeasons.includes(season)).map((season) => ({ season, reason: 'outside_requested_range' }));
  await writeJson(resolve(outputRoot, 'BLOCK45_REPORT.json'), { ...baseReport, status: stoppedForBudget ? 'plan_budget_insufficient' : providerErrors > 0 ? 'plan_partial' : 'plan_ready', planOnly, noSnapshotsCreated: true, noDatabaseTouched: true, discoveredSeasonsByLeague: Object.fromEntries([...discoveredByLeague.entries()].map(([id, years]) => [id, years])), selectedSeasons: requestedSeasons, activeSeason, requestsEstimated: totalEstimated, requestsEstimatedBreakdown: { leagueCoverage: discoveredByLeague.size, leaguePages: pagesEstimated, playerSeasonExpansion: expansionEstimate }, requestsPerformed: requests.length, quotaInitial: requests[0]?.quotaRemaining ?? null, quotaFinal: requests.at(-1)?.quotaRemaining ?? null, pages: requests.filter((request) => request.kind === 'league_page').length, candidatePlayers: playerIds.size, factsObserved: uniqueFacts.length, duplicatesRemoved: facts.length - uniqueFacts.length, providerErrors, stoppedForBudget, coverage, competitionsNotQueried: notQueried, playerCompetitionCoverageFile: 'BLOCK45_PLAYER_COVERAGE.json', playerSeasonCoverageFile: 'BLOCK45_PLAYER_SEASON_COVERAGE.json', controls: candidate.controls, rankingCandidate: { careerTop100: candidate.snapshots.career.ranking.slice(0, 100), activeSeasonTop100: candidate.snapshots.active_season.ranking.slice(0, 100), activePlayersCareerTop100: candidate.snapshots.active_players_career.ranking.slice(0, 100) }, hashes: { facts: sha256(stableYellowJson(uniqueFacts.map((fact) => fact.id).sort())), requests: sha256(stableYellowJson(requests.map((request) => request.responseSha256))) }, rawPayloadsStored: false, secretPrinted: false, snapshotsCreated: 0, officialSnapshotCreated: false });
}
